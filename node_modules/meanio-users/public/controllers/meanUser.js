...
angular.module('mean.users')
  ...
  .controller('LoginCtrl', ['$rootScope','$scope','MeanUser', 'vcRecaptchaService',
    function($rootScope, $scope, MeanUser, vcRecaptchaService) {
      var vm = this;

      vm.loginattemptfail = 0;
      ...
      $rootScope.$on('loginfailed', function(){
          //vm.user = {};
          vm.loginattemptfail += 1;
          vm.loginError = MeanUser.loginError;
          if (vm.loginattemptfail > 2) {
              vcRecaptchaService.reload(vm.recaptcha_widgetId);
          }
      });
      // Register the login() function
      vm.login = function() {
          if(vm.loginattemptfail > 1) {
              if (vm.recaptcha_response === "") { //if string is empty
                  alert("Please resolve the captcha and submit!")
                  vcRecaptchaService.reload();
              } else {
                  MeanUser.login(this.user,vm.recaptcha_response);
              }
          }else{
              MeanUser.login(this.user,vm.recaptcha_response);
          }
...
*/
  ...
  .controller('UserProfileEditCtrl', ['MeanUser','$rootScope','$scope','$mdDialog',
      function(MeanUser,$rootScope,$scope,$mdDialog) {
